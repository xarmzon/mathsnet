...
export interface IRegFormData {
  fullName: IFormDataValue;
  username: IFormDataValue;
  email: IFormDataValue;
  password: IFormDataValue;
  cPassword: IFormDataValue;
}

const sbmtTxt = "Add Instructor";
const Instructors = () => {
  useUserType();
  ...
  const { data: instructorsData, error: instructorsDataError } = useSWR(
    `${ROUTES.API.INSTRUCTOR}?get_type=instructor&search=${searchVal}&page=${page}`
  );

  //if (instructorsData) console.log(instructorsData);
  const resetMessage = () => {
    handleResMsg();
  ...
  const handleEdit = (id: string) => {
    resetMessage();
    //console.log(id);
    const data = instructorsData?.results?.filter((v) => v._id === id)[0];
    //console.log(data);
    if (data) {
    ...
    if (confirm("Are you sure?")) {
      try {
        ...
        const { data: deleteRes } = await api.delete(
          `${ROUTES.API.INSTRUCTOR}?id=${id}&delete_type=${"instructor"}`
        );
        mutate(
          `${
            ROUTES.API.INSTRUCTOR
          }?get_type=instructor&search=${searchVal}&page=${1}`
        );
  ...
  const handleForm = async (e: any) => {
    ...
    if (errors.length > 0) {
      ...
      try {
        ...
        formDataForward["instructor"] = true;
        formDataForward["post_type"] = "instructor";
  ...
  return (
    <DashboardLayout>
      ...
      >
        <p className="text-gray-400 ">New Instructor</p>
      ...
      <div className="mt-8 space-y-3 overflow-hidden">
        <p className="text-gray-400 ">Instructors List</p>
        <DataTable
          header={instructorListHeader}
          showEdit={false}
          data={
            !instructorsDataError && instructorsData
              ? [
                  ...instructorsData?.results?.map((d) => ({
                    id: d._id,
                    values: [
          ...
          page={page}
          perPage={
            instructorsData?.paging?.perPage
              ? instructorsData?.paging?.perPage
              : PER_PAGE
          }
          totalPage={
            instructorsData?.paging?.totalPages
              ? instructorsData?.paging?.totalPages
              : 1
          }
          message={message}
